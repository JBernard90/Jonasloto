...
export default function POS() {
  ...
  const fetchRecentSales = async () => {
    try {
      ...
      // Calculate session total (mocking session as today's sales for now)
  ...
  const handleSale = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setLastTicket({ id: data.id, ...ticketData });
      ...
      console.log("Printing ticket...", data.id);
    } catch (error) {
      ...
      <div className="lg:col-span-2 space-y-8">
        ...
        <div className="bg-white p-8 rounded-3xl shadow-sm border border-gray-100">
          <div className="flex items-center justify-between mb-6">
            ...
            <div className="text-xs font-bold text-gray-400 uppercase tracking-widest">Total Session: {sessionTotal} HTG</div>
          </div>
          <div className="space-y-4">
            {recentSales.length > 0 ? (
              recentSales.map((ticket, i) => (
                <div key={ticket.id || i} className="flex items-center justify-between p-4 bg-gray-50 rounded-2xl border border-gray-100">
                  <div className="flex items-center gap-4">
                    <div className="w-10 h-10 bg-white rounded-xl flex items-center justify-center text-primary shadow-sm">
                      <Ticket size={20} />
                    </div>
                    <div>
                      <div className="text-sm font-bold text-gray-900">{ticket.borlette}</div>
                      <div className="text-[10px] text-gray-400">
                        {formatDistanceToNow(new Date(ticket.createdAt), { addSuffix: true, locale: fr })} • ID: #{ticket.id?.slice(0, 8)}
                      </div>
                    </div>
                  </div>
                  <div className="flex gap-2">
                    {/* Display first few numbers if available */}
                    {Object.values(ticket.entries || {})[0] && (Object.values(ticket.entries || {})[0] as any).numbers.slice(0, 2).map((num: string, idx: number) => (
                      <span key={idx} className="w-6 h-6 rounded-full bg-gray-900 text-white flex items-center justify-center font-bold text-[10px]">{num}</span>
                    ))}
                  </div>
                  <div className="text-right">
                    <div className="text-sm font-black text-gray-900">{ticket.amount} HTG</div>
                    <div className="text-[10px] text-green-500 font-bold uppercase tracking-widest">{ticket.paymentMethod || 'Cash'}</div>
                  </div>
                </div>
              ))
            ) : (
              <div className="text-center py-8 text-gray-400 font-bold text-sm italic">Aucune vente récente</div>
            )}
          </div>
        </div>
      </div>
