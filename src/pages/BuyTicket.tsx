...
export default function BuyTicket({ user }: BuyTicketProps) {
  ...
  const handleVerifyOTP = async () => {
    ...
    try {
      ...
      await updateDoc(doc(db, 'otps', otpDoc.id), { status: 'used' });
      handlePayment();
    } catch (error) {
          ...
          >
            ...
            <div className="flex justify-between">
              ...
              {!otpSent ? (
                ...
                >
                  {otpLoading ? <div className="w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin"></div> : <CreditCard size={20} />}
