...
export default function Results() {
  ...
  useEffect(() => {
    let q = query(collection(db, 'draws'), orderBy('date', 'desc'));
    
    if (filterType !== 'all') {
      q = query(collection(db, 'draws'), where('type', '==', filterType), orderBy('date', 'desc'));
    }
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      if (searchDate) {
        results = results.filter(draw => {
          const drawDate = draw.date?.toDate ? format(draw.date.toDate(), 'yyyy-MM-dd') : '';
      ...
      handleFirestoreError(error, OperationType.LIST, 'draws');
    });
                ...
                <tr key={draw.id} className="hover:bg-gray-50 transition-colors group">
                  <td className="px-6 py-6">
                    <div className="text-sm font-bold text-gray-900">
                      {draw.date?.toDate ? format(draw.date.toDate(), 'PPP', { locale: getDateLocale() }) : ''}
                    </div>
                    <div className="text-[10px] text-gray-400">
                      {draw.date?.toDate ? format(draw.date.toDate(), 'HH:mm') : ''}
                    </div>
                  </td>
                  <td className="px-6 py-6">
