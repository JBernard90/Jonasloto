...
import { User, signInWithPopup, GoogleAuthProvider, signOut, createUserWithEmailAndPassword, signInWithEmailAndPassword, deleteUser } from 'firebase/auth';
import { collection, query, where, onSnapshot, doc, setDoc, getDoc, updateDoc, Timestamp, orderBy, deleteDoc } from 'firebase/firestore';
  ...
  useEffect(() => {
    ...
    const fetchProfile = async () => {
      ...
      if (userDoc.exists()) {
        ...
        setFullName(newProfile.displayName);
      }
    };
    fetchProfile();
  ...
  const handleDeleteSelf = async () => {
    ...
    try {
      ...
      try {
        ...
        // If it fails due to recent login requirement, just sign out
        // The document is already gone, so they effectively can't use the app
        ...
        <div className="bg-white p-8 rounded-3xl shadow-sm border border-gray-100 text-center relative overflow-hidden">
          ...
          <div className="relative inline-block mb-4">
            <div className="w-24 h-24 bg-primary/10 text-primary rounded-3xl flex items-center justify-center mx-auto border border-primary/20">
              {user.photoURL ? (
                <img src={user.photoURL} alt="Avatar" className="w-full h-full rounded-3xl object-cover" />
              ) : (
                <UserIcon size={40} />
              )}
            </div>
          ...
          {isEditing ? (
            ...
            <>
              <h2 className="text-2xl font-black text-gray-900 tracking-tight uppercase italic">{userData?.displayName || user.displayName}</h2>
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <div className="text-[10px] text-gray-400 font-bold uppercase tracking-widest">
                        {ticket.createdAt?.toDate ? format(ticket.createdAt.toDate(), 'PPP', { locale: getDateLocale() }) : ''}
                      </div>
                    </div>
                  </div>
